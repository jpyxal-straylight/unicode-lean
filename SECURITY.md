...
The artefacts that downstream consumers rely on are:
...
byte-identical `.olean` artefacts — is verified by the nightly
...
SHA-256 hashes as a build artefact (`oleans-sha256-<commit-sha>`) so
...
Source-of-truth artefacts — the UCD source files under `Unicode/Ucd/`,
